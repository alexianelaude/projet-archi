...
int main(){
    ...
    printf("VALEURS\nSéquentiel (scalaire: %e, vectoriel: %e) Parallèle (nb_threads: %d, scalaire: %e, vectoriel: %e) \n", res_ref, res_vec, NB_THREADS, res_par, res_par_vec);
    printf("TEMPS D'EXECUTION\nSéquentiel (scalaire: %e, vectoriel: %e) Parallèle (nb_threads: %d, scalaire: %e, vectoriel: %e) \n", time_ref, time_vec, NB_THREADS, time_par, time_par_vec);
    printf("Accélération (vectoriel: %e, multithread: %e, vectoriel + multithread: %e)", acc_vec, acc_par, acc_par_vec);
}
